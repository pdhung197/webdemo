...
function dataValidate(forminputitem, value) {
    ...
    switch (datatype) {
        case 'number':
            ...
            break;
        case 'email':
            dataValidateBool = validateDataEmail(forminputitem, value);
            break;
        default:
            break;
    }
...
function validateDataEmail(forminputitem, value) {
    let emailform = /[a-zA-Z0-9]+@[a-zA-Z0-9_]+(.[a-z]{2,})+$/gi;
    let emailtest = emailform.test(value);
    if (!emailtest) errorTextSet(forminputitem, 'wrong');
    return emailtest;
}
